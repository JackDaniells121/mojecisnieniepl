...
class RollCounterRange {
	...
	changeValue() {
		...
		if (this.fill) {
			...
			// 	fillWidth = pct * 69,
			// 	thumbEm = 1 - pct;
            let pct = 1 / (this.el.max - this.el.min) ,
				fillWidth = pct * (this.el.value - this.el.min) * 100;
            this.fill.style.height = '2rem';
            this.el.style.height = '2rem';
		...
		if (this.digitCols) {
            ...
            // document.getElementById('bloodPressure1').value=rangeVal
		}
	}
}